...
local Luna = { 
	Folder = "Luna", 
	Options = {}, 
	ThemeGradient = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(230, 75, 75)),ColorSequenceKeypoint.new(1.00, Color3.fromRGB(80, 15, 15))} 
}
...
local function Hide(Window, bind, notif)
	local function _hide()
		-- Defensive hide: the UI may not yet be fully constructed when this is called
		if not Window or typeof(Window) ~= "Instance" then
			Window = Main
		end

		pcall(function() SizeBleh = Window.Size end)
		local bindName = ""
		pcall(function()
			bindName = string.split(tostring(bind), "Enum.KeyCode.")
			bindName = bindName[2]
		end)

		if notif then
			pcall(function()
				Luna:Notification({Title = "Interface Hidden", Content = "Re-open Window By Pressing ["..tostring(bindName).."]", Icon = "visibility_off", Duration = 1})
			end)
		end

	pcall(function() Window.BackgroundTransparency = 1 end)
	if Window:FindFirstChild("Elements") then pcall(function() Window.Elements.BackgroundTransparency = 1 end) end
	if Window:FindFirstChild("Line") then pcall(function() Window.Line.BackgroundTransparency = 1 end) end
	if Window:FindFirstChild("Title") and Window.Title:FindFirstChild("Title") then pcall(function() Window.Title.Title.TextTransparency = 1 end) end
	if Window:FindFirstChild("Title") and Window.Title:FindFirstChild("subtitle") then pcall(function() Window.Title.subtitle.TextTransparency = 1 end) end
	if Window:FindFirstChild("Logo") then pcall(function() Window.Logo.ImageTransparency = 1 end) end
	if Window:FindFirstChild("Navigation") and Window.Navigation:FindFirstChild("Line") then pcall(function() Window.Navigation.Line.BackgroundTransparency = 1 end) end

		if Window:FindFirstChild("Controls") then
			for _, TopbarButton in ipairs(Window.Controls:GetChildren()) do
				if TopbarButton.ClassName == "Frame" then
					pcall(function() TopbarButton.BackgroundTransparency = 1 end)
					if TopbarButton:FindFirstChild("UIStroke") then pcall(function() TopbarButton.UIStroke.Transparency = 1 end) end
					if TopbarButton:FindFirstChild("ImageLabel") then pcall(function() TopbarButton.ImageLabel.ImageTransparency = 1 end) end
					pcall(function() TopbarButton.Visible = false end)
				end
			end
		end

		if Window:FindFirstChild("Navigation") and Window.Navigation:FindFirstChild("Tabs") then
			for _, tabbtn in ipairs(Window.Navigation.Tabs:GetChildren()) do
				if tabbtn.ClassName == "Frame" and tabbtn.Name ~= "InActive Template" then
					pcall(function() tabbtn.BackgroundTransparency = 1 end)
					if tabbtn:FindFirstChild("ImageLabel") then pcall(function() tabbtn.ImageLabel.ImageTransparency = 1 end) end
					if tabbtn:FindFirstChild("DropShadowHolder") and tabbtn.DropShadowHolder:FindFirstChild("DropShadow") then pcall(function() tabbtn.DropShadowHolder.DropShadow.ImageTransparency = 1 end) end
					if tabbtn:FindFirstChild("UIStroke") then pcall(function() tabbtn.UIStroke.Transparency = 1 end) end
				end
			end
		end

	task.wait(0.00)
	pcall(function() Window.Size = UDim2.new(0,0,0,0) end)
		if Window.Parent and Window.Parent:FindFirstChild("ShadowHolder") then pcall(function() Window.Parent.ShadowHolder.Visible = false end) end
	task.wait()
	if Window:FindFirstChild("Elements") and Window.Elements.Parent then pcall(function() Window.Elements.Parent.Visible = false end) end
	pcall(function() Window.Visible = false end)
	end

	local ok, err = xpcall(_hide, function(e)
		warn("[Luna] Hide error: ", e)
...
function Luna:Notification(data) -- action e.g open messages
	task.spawn(function()
		...
		local iconRes = GetIcon(data.Icon, data.ImageSource)
		if type(iconRes) == "string" then
			newNotification.Icon.Image = iconRes
			-- clear any rect overrides
			pcall(function()
				newNotification.Icon.ImageRectSize = Vector2.new(0, 0)
				newNotification.Icon.ImageRectOffset = Vector2.new(0, 0)
			end)
		elseif type(iconRes) == "table" and iconRes.id then
			newNotification.Icon.Image = "rbxassetid://" .. tostring(iconRes.id)
			if iconRes.imageRectSize then
				pcall(function() newNotification.Icon.ImageRectSize = iconRes.imageRectSize end)
			end
			if iconRes.imageRectOffset then
				pcall(function() newNotification.Icon.ImageRectOffset = iconRes.imageRectOffset end)
			end
		else
			-- fallback placeholder
			newNotification.Icon.Image = "rbxassetid://10723434557"
		end
...
local function Unhide(Window, currentTab)
    local function _unhide()
        -- Defensive unhide: only operate on children that actually exist
        if not Window or typeof(Window) ~= "Instance" then
            Window = Main
        end
		...
		-- Immediate (non-animated) unhide: set properties directly
		pcall(function() Window.BackgroundTransparency = 0.2 end)
		if elements then pcall(function() elements.BackgroundTransparency = 0.08 end) end
		local line = Window:FindFirstChild("Line")
		if line then pcall(function() line.BackgroundTransparency = 0 end) end
		local title = Window:FindFirstChild("Title")
		if title then
			local titleText = title:FindFirstChild("Title")
			if titleText then pcall(function() titleText.TextTransparency = 0 end) end
			local subtitle = title:FindFirstChild("subtitle")
			if subtitle then pcall(function() subtitle.TextTransparency = 0 end) end
		end
		local logo = Window:FindFirstChild("Logo")
		if logo then pcall(function() logo.ImageTransparency = 0 end) end
		local navigation = Window:FindFirstChild("Navigation")
		if navigation then
			local navLine = navigation:FindFirstChild("Line")
			if navLine then pcall(function() navLine.BackgroundTransparency = 0 end) end
		end

		local controls = Window:FindFirstChild("Controls")
		if controls then
			for _, TopbarButton in ipairs(controls:GetChildren()) do
				if TopbarButton.ClassName == "Frame" and TopbarButton.Name ~= "Theme" then
					pcall(function() TopbarButton.Visible = true end)
					pcall(function() TopbarButton.BackgroundTransparency = 0.25 end)
					local uiStroke = TopbarButton:FindFirstChild("UIStroke")
					if uiStroke then pcall(function() uiStroke.Transparency = 0.5 end) end
					local imageLabel = TopbarButton:FindFirstChild("ImageLabel")
					if imageLabel then pcall(function() imageLabel.ImageTransparency = 0.25 end) end
				end
			end
		end

		if navigation then
			local tabs = navigation:FindFirstChild("Tabs")
			if tabs then
				for _, tabbtn in ipairs(tabs:GetChildren()) do
					if tabbtn.ClassName == "Frame" and tabbtn.Name ~= "InActive Template" then
						if tabbtn.Name == currentTab then
							pcall(function() tabbtn.BackgroundTransparency = 0 end)
							local uiStroke = tabbtn:FindFirstChild("UIStroke")
							if uiStroke then pcall(function() uiStroke.Transparency = 0.41 end) end
						end
						local imageLabel = tabbtn:FindFirstChild("ImageLabel")
						if imageLabel then pcall(function() imageLabel.ImageTransparency = 0 end) end
						...
						if dropShadowHolder then
							...
							if dropShadow then pcall(function() dropShadow.ImageTransparency = 1 end) end
						end
					end
				end
			end
    ...
    local ok, err = xpcall(_unhide, function(e)
        warn("[Luna] Unhide error: ", e)
        warn(debug.traceback())
        if Window and typeof(Window) == "Instance" then
            Window.Visible = true
        end
...
function Luna:CreateWindow(WindowSettings)
	...
	-- Ensure thumbnail has a consistent size/position for better centering
	pcall(function()
		local img = Navigation.Player.icon:FindFirstChild("ImageLabel")
		if img then
			img.Size = UDim2.new(0, 38, 0, 38)
			img.Position = UDim2.new(0.5, -20, 0.5, -20)
			-- img.AnchorPoint = Vector2.new(0.5, 0.5)
			-- clear any stale ImageRect overrides
			pcall(function()
	...
	function Window:CreateTab(TabSettings)
		...
		function Tab:CreateSection(name : string)
			...
			function Section:CreateDropdown(DropdownSettings, Flag)
				...
				SafeCallback(bleh)
				-- `bleh` may be a string or a table: handle both safely
				...
				function DropdownV:Set(NewDropdownSettings)
					...
					-- `bleh` may be a string (single selection) or a table (multiple selection)
					if type(bleh) == "string" then
						if Dropdown.List:FindFirstChild(bleh) then
							tween(Dropdown.List[bleh], {TextColor3 = Color3.fromRGB(240,240,240), BackgroundTransparency = 0.95})
						end
		...
		function Tab:CreateDropdown(DropdownSettings, Flag)
			...
			function DropdownV:Set(NewDropdownSettings)
				...
				-- safe set: bleh may be string or table
				if type(bleh) == "string" then
					if Dropdown.List:FindFirstChild(bleh) then
						tween(Dropdown.List[bleh], {TextColor3 = Color3.fromRGB(240,240,240), BackgroundTransparency = 0.95})
					end
	...
	UserInputService.InputBegan:Connect(function(input, gpe)
		...
		if input.KeyCode == Window.Bind then
			-- Toggle visibility: if open -> hide, if closed -> unhide
