			...
			if type(value) == "table" then
				if value.__type == "ColorSequence" then
					...
					for _, kp in ipairs(value.Keypoints) do
						table.insert(keypoints, ColorSequenceKeypoint.new(kp.Time, Color.Pack(kp.Value)))
					end
...
if PlayerGui:FindFirstChild("TouchGui") then
	function check()
		if PlayerGui:FindFirstChild("TouchGui").TouchControlFrame.JumpButton.Visible then
			StarlightUI.Notifications.Position = UDim2.new(
				1,
				-20,
				1,
				-(24 + PlayerGui:FindFirstChild("TouchGui").TouchControlFrame.JumpButton.AbsoluteSize.Y)
			)
		else
		...
		if WindowSettings.LoadingEnabled then
			...
			Tween(StarlightUI.Drag, {
				Position = UDim2.new(
					...
					0,
					0,
					((Camera.ViewportSize.Y / 2 - GuiInset) - size.Y.Offset / 2) - (GuiInset / 2) + size.Y.Offset + 10
				),
			}, nil, Tween.Info(nil, nil, 1.1))
			...
			function Tab:BuildConfigGroupbox(Column, Style, ButtonsCentered)
				...
				instance:CreateButton({
					...
					Callback = function()
						...
						if
							isfile(
								`{Starlight.FileSystem.Folder}/{folderpath}/configs/{inputPath}{Starlight.FileSystem.FileExtension}`
							)
						then
							Starlight:Notification({
...
function Starlight.FileSystem:BuildFolderTree(hasRoot: boolean, Root: string, Folder: string, hasTheme: boolean?)
	...
	if isfolder(`{Starlight.FileSystem.Folder}/Configurations/{path}`) then
		ConfigMethods.UpdateOld(
			`{Starlight.FileSystem.Folder}/Configurations/{path}`,
			`{Starlight.FileSystem.Folder}/{path}/configs`
		)
	end
end
...
function Starlight.FileSystem:RefreshConfigList(path)
	...
	if not isfolder(path) then
		print("dawg get a better executor")
		return false
	end
...
if isStudio and enabled then
	...
	Starlight:SetTheme(readfile(`{themesPath}/autoload.txt`))
end --]=]
